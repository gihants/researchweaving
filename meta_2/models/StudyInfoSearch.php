<?php

namespace app\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\StudyInfo;

/**
 * StudyInfoSearch represents the model behind the search form of `app\models\StudyInfo`.
 */
class StudyInfoSearch extends StudyInfo
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['study_ID', 'FirstAuthor', 'title', 'Country', 'Year', 'Journal'], 'safe'],
            [['Volume'], 'integer'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = StudyInfo::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'Year' => $this->Year,
            'Volume' => $this->Volume,
        ]);

        $query->andFilterWhere(['like', 'study_ID', $this->study_ID])
            ->andFilterWhere(['like', 'FirstAuthor', $this->FirstAuthor])
            ->andFilterWhere(['like', 'title', $this->title])
            ->andFilterWhere(['like', 'Country', $this->Country])
            ->andFilterWhere(['like', 'Journal', $this->Journal]);

        return $dataProvider;
    }
}
