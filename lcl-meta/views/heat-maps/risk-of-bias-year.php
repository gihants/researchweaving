...
$years_view = RiskOfBiasYear::find()
    ->asArray()
    ->all();
...
for ($i = $min_year; $i<=$max_year; $i++) {
    ...
    $a_articles = RiskOfBiasYear::find()
        ->select('articles')
        ->andWhere(['year' => $i])
        ->andWhere(['risk_of_bias' => 'low'])
        ->asArray()
        ->all();
    ...
    $b_articles = RiskOfBiasYear::find()
        ->select('articles')
        ->andWhere(['year' => $i])
        ->andWhere(['risk_of_bias' => 'medium'])
        ->asArray()
        ->all();
    ...
    $c_articles = RiskOfBiasYear::find()
        ->select('articles')
        ->andWhere(['year' => $i])
        ->andWhere(['risk_of_bias' => 'high'])
        ->asArray()
        ->all();
...
$this->title = "Risk of bias by year";
$this->params['breadcrumbs'][] = ['label' => 'Risk of bias by year', 'url' => ['index']];
...
<p>The following diagram illustrate the distribution of risk of bias among different publication years.</p>
        ...
        series: [ {
            ...
            events: {
                click: function (event) {
                    ...
                    var url_to = <?php echo json_encode(Url::to(['material-info-view/risk-level-year'])); ?>;
                    var new_url = url_to.concat('&risk_level=','high','&year=', year);
            ...
            events: {
                click: function (event) {
                    ...
                    console.log(event.point);
                    var url_to = <?php echo json_encode(Url::to(['material-info-view/risk-level-year'])); ?>;
                    var new_url = url_to.concat('&risk_level=','medium','&year=', year);
                    var win = window.open(new_url, '_blank');
                    win.focus();
                }
            }
            ...
            events: {
                click: function (event) {
                    ...
                    console.log(event.point);
                    var url_to = <?php echo json_encode(Url::to(['material-info-view/risk-level-year'])); ?>;
                    var new_url = url_to.concat('&risk_level=','low','&year=', year);
                    var win = window.open(new_url, '_blank');
                    win.focus();
                }
            }
